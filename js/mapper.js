...
const transform = (obj, mapping, direction) => {
    ...
    if (isPresent(val)) {
      if(item.$mapping){
        ...
        val = val.map((v)=> {
          ...
          if(lastRule.$filter){
            return Object.assign({}, tv, clone(lastRule.$filter));
          }
          return tv;
        });
      }
...
const Mapper = {
  ...
  inject: inject,
  transform: transform,
  fhirToForm: (data, mapping) => transform(data, mapping, ["fhir", "form"]),
  formToFhir: (data, mapping) => transform(data, mapping, ["form", "fhir"])
};
